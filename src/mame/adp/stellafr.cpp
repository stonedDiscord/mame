...
public:
	...
	void sus_common(machine_config &config) ATTR_COLD;
...
void stellafr_state::sus_common(machine_config &config)
{
	...
	steuereinheit.duart_output_callback().set(FUNC(stellafr_state::duart_output_w));
	psg.port_a_read_callback().set_ioport("IN0");
	psg.port_b_write_callback().set(FUNC(stellafr_state::ay8910_portb_w));
}
...
ROM_START( jkrpoker )
	ROM_REGION16_BE( 0x300000, "maincpu", 0 )
